...
- 実装完了後、私に報告する前に下記を確認すること
  - `frontend/` 以下を変更した場合は、 `npm run check:format` `npm run typecheck` `npm test` `npm run build`
...
- Codexでの実行時は、binaryenのダウンロードエラーを避けるために `wasm-pack build` に `--dev` オプションをつけて実行する。
